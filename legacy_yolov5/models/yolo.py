...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #self.m = nn.ModuleList(DecoupledHead(x, nc, 1, anchors) for x in ch) # DecoupledHead
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, GSConv, VoVGSCSP, DSConv2D, C3_DSConv, C3_Faster}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, VoVGSCSP, C3x, C3_DSConv, C3_Faster}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='1.yaml', help='model.yaml')
